...
describe('Planet', function() {
  ...
  it('should not grant travel clearance if fuel is less than planet distance', function() {
    var planet = new Planet({ name: 'F2', coordinates: { x: 0, y: 0, z: 0 } });
    var otherPlanet = new Planet({ name: 'M2', coordinates: { x: 1, y: 1, z: 1 }});
    var captain = new Being('Will', 'human');
    var parts = {
    ...
    var result = planet.giveClearance(otherPlanet)
    assert.equal(result, `Clearance denied: Fuel level must be equal to or greater than travel distance: ${planet.calculateDistance(otherPlanet)}`);
    planet.refuel(planet.currentShip);

    var result2 = planet.giveClearance(otherPlanet);
    assert.equal(fighter.fuel, 0);
    assert.equal(planet.currentShip, undefined);
    assert.equal(otherPlanet.currentShip, fighter);
    assert.equal(result2, `Clearance granted: Enjoy your trip to ${otherPlanet.name}`);
  });
})
